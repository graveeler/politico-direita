...
/**
 * Função que  retorna um diretorio apartir da pasta imagens
 */
function imagesPath(?string $uri = null)
{
    return BASE_PATH . "/cdn/assets/images/{$uri}";
}
...
function objectToArray($object, ...$secret): array
{
    $teste = [];
    if ($object == null) {
        return (array)$teste;
...
{
    if ($respawnTime == 0) {
        return '<h6 class="badge bg-success">Alive</h6>';
    }

    return '<h6 class="badge bg-danger">Dead</h6>';
}
...
function redimensionarImagem($caminhoImagem, $largura, $altura)
{
    ...
    // Salva a nova imagem em um arquivo temporário
    $caminhoImagemRedimensionada = tempnam(sys_get_temp_dir(), 'redimensionada_') . '.png';
    imagepng($novaImagem, $caminhoImagemRedimensionada);

    // Libera a memória
    imagedestroy($imagemOriginal);
    imagedestroy($novaImagem);

    return $caminhoImagemRedimensionada;
}
